...
function generateRandomCoordinates(
	center: LatLngExpressionArray,
	radius: number,
	count: number
): LatLngExpressionArray[] {
	...
	for (let i = 0; i < count; i++) {
		...
		const lng =
			center[1] +
			(Math.random() - 0.5) * (radius / (111.32 * Math.cos((center[0] * Math.PI) / 180))); // 1 degree of longitude is approximately 111.32 kilometers * cos(latitude)
